// 获取元素的CSS样式
function getCss(element: HTMLElement, key: string): string {
  return (element as any).currentStyle
    ? (element as any).currentStyle[key]
...
// 初始化拖拽
export function initDrag(bar: HTMLElement | null, box: HTMLElement | null): void {
  if (!bar || !box)
    return
