...
function formatterType(type: number) {
  let account = ''
  if (type === 0) account = '永久账号'

  if (type === 1) account = '临时账号'
...
<template>
  ...
  >
    ...
    <div
      v-if="props.detailData.validityPeriodType === 1"
      class="flex items-center justify-start mb-10px"
    >
