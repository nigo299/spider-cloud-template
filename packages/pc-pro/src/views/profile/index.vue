...
const [pwdFormRef, pwdForm, pwdValidation] = useForm<PasswordForm>()

async function handlePwdSave(): Promise<boolean | void> {
  await pwdValidation()
...
const newAvatar = ref<string>(userStore.avatar || '')
const [avatarModalRef] = useModal()
async function handleAvatarSave(): Promise<boolean | void> {
  if (!newAvatar.value) {
    window.$message.error('请输入头像地址')
    return false
  }
...
const [profileFormRef, profileForm, profileValidation] = useForm<ProfileForm>({
  id: userStore.userId || '',
