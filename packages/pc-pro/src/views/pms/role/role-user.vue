...
const columns: DataTableColumns<UserItem> = [
  ...
  },
  {
    ...
    fixed: 'right',
    render(row: UserItem) {
      return row.roles?.some(item => item.id === +(route.params.roleId as string))
        ? h(
            NButton,
            {
              size: 'small',
...
function handleBatchAdd(ids: (string | number)[] = userIds.value): void {
  const roleId = route.params.roleId
  if (!roleId)
    return window.$message.error('角色异常，请重新选择角色')
  if (!ids.length)
    return window.$message.error('请先选择用户')
...
function handleBatchRemove(ids: (string | number)[] = userIds.value): void {
  const roleId = route.params.roleId
  if (!roleId)
    return window.$message.error('角色异常，请重新选择角色')
  if (!ids.length)
