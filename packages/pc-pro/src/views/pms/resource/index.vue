  ...
  <CommonPage>
    <div class="flex">
      <n-spin size="small" :show="treeLoading">
        <MenuTree
          v-model:current-menu="currentMenu as unknown as ResourceItem"
...
const currentMenu = ref<ResourceItem | null>(null)

async function initData(data?: ResourceItem): Promise<void> {
  if (data?.type === 'BUTTON') {
...
function handleEdit(item: ResourceItem): void {
...
const btnsColumns: DataTableColumns<ResourceItem> = [
  { title: '名称', key: 'name' },
  { title: '编码', key: 'code' },
  {
    title: '状态',
...
function handleEditBtn(row: ResourceItem): void {
  modalRef.value?.handleOpen({
    action: 'edit',
    title: `编辑按钮 - ${row.name}`,
    row,
...
function handleDeleteBtn(id: string | number): void {
  const d = window.$dialog.warning({
    ...
    async onPositiveClick() {
      try {
        d.loading = true
        await api.deletePermission(id.toString())
...
async function handleEnable(item: ResourceItem): Promise<void> {
  try {
    item.enableLoading = true
    await api.savePermission(item.id.toString(), {
      enable: !item.enable,
    })
