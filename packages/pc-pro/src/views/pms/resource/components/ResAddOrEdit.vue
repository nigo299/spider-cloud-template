...
const menuOptions = computed<TreeOption[]>(() => {
...
const defaultForm: Partial<ResourceItem> = { enable: true, show: true, layout: '' }
const [modalFormRef, modalForm, validation] = useForm<ResourceItem>()
const [modalRef, okLoading] = useModal()

const modalAction = ref('')
const parentIdDisabled = ref(false)
function handleOpen(options: OpenOptions = { action: '' }) {
  const { action, row = {} as Partial<ResourceItem>, ...rest } = options
  modalAction.value = action
  modalForm.value = { ...defaultForm, ...row } as ResourceItem
...
async function onSave() {
  ...
  try {
    ...
    if (modalAction.value === 'add') {
      ...
      await api.savePermission(modalForm.value.id!.toString(), modalForm.value)
    }
