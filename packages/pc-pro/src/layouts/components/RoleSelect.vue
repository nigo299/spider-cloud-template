...
const roles = ref<Role[]>(userStore.roles || [])
const roleCode = ref<string>(userStore.currentRole?.code ?? roles.value[0]?.code ?? '')
...
async function setCurrentRole(): Promise<boolean | void> {
  try {
    okLoading.value = true
    const { data } = await api.switchCurrentRole(roleCode.value)
    const tokenData: TokenData = {
      accessToken: data.success ? authStore.accessToken || '' : '',
    }
    await authStore.switchCurrentRole(tokenData)
